...
def process_annotation_Entity_Span(annotation, sent, token_list):
    ...
    for entity in delete_entity_list:
        try:
            ...
            pass
    # for (anno, num), entity in zip(delete_list_dict.items(), delete_entity_list):
    #     try:
    #
    #         annotation.remove(eval(anno))
    #         exist_each_entity_list.remove(entity)
    #     except ValueError:
    #         pass
...
def process_annotation_Entity_type(annotation, sent, token_list, entity_type_flag):
    ...
    for anno_item in annotation:
        ...
        for entity in entity_list:
            ...
            # if entity_type=="drug" or entity_type=="group" or entity_type=="brand"  or entity_type=="drug_n":
            entity_type = "Drug"
...
def process_annotation_Relation_type(annotation, sent, token_list, relation, entity_list):
    ...
    for anno_item in annotation:
        if len(anno_item) == 4:
            ...
            # ["Drug_Drug_interaction", "Drug_Chemical_interaction"]
            # ["mechanism", "effect", "advise", "int"]

            if relation_tag=="mechanism" or relation_tag=="effect" or relation_tag=="advise" or relation_tag=="int":
                relation_tag = "Drug_Drug_interaction"

                # if (anno_item[0][3] =="drug_n" and anno_item[1][3] in ["drug", "group", "brand"]) or \
                #         (anno_item[1][3] =="drug_n" and anno_item[0][3] in ["drug", "group", "brand"]):
                #     relation_tag = "Drug_Chemical_interaction"
...
def process_data(raw_file, tokenizer, res_file, entity_type_list, relation_type_list, train_test_name):
    with open(raw_file, "r") as f:
        raw_data = f.readlines()

    if train_test_name=="train":
        train_len = int(len(raw_data)*0.9)
        train_raw_data = raw_data[:train_len]
        valid_raw_data = raw_data[train_len:]
        raw_data_list = [train_raw_data, valid_raw_data]
        raw_res_list = [res_file, res_file.replace("train", "valid")]
    else:
        raw_data_list = [raw_data]
        raw_res_list = [res_file]

    for raw_data_item, res_file in zip(raw_data_list, raw_res_list):
        with tqdm(total=len(raw_data_item)) as pbar:
            with open(res_file, "a") as f:
                for sent_index in range(len(raw_data_item)):
                    pbar.update(1)
                    temp_res = raw_data_item[sent_index].split("||")
                    ...
                    for entity_type in entity_type_list:
                        joint_gold_list, only_entity_type = process_annotation_Entity_type(new_annotation, sent, token_list,
                                                                                           entity_type)
...
if __name__ == '__main__':
    ...
    file_list = [["Train_MedLine.csv", "Train_DrugBank.csv"],
                 ["Test_MedLine.csv", "Test_DrugBank.csv"]]

    entity_type_list = ["Drug"]
    relation_type_list = ["Drug_Drug_interaction"]

    for file_type in file_list:
        for index, file in enumerate(file_type):
            raw_file = os.path.join(dir_raw, file)
            train_test_name = file.split("_")[0].lower()
            res_file = os.path.join(dir_res, "DDI_" + train_test_name + "_" + bert_model + "_model_data.json")
            if index == 0:
                if os.path.exists(res_file):
                    os.remove(res_file)
                    if train_test_name == "train":
                        valid_file = res_file.replace("train", "valid")
                        try:
                            os.remove(valid_file)
                        except:
                            pass
                        print(str(valid_file) + " has deleted!!")
                    print(res_file + " has deleted!!")
            process_data(raw_file, tokenizer, res_file, entity_type_list, relation_type_list, train_test_name)
