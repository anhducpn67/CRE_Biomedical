...
def process_annotation_Entity_Span(annotation, sent, token_list):
        ...
        for entity in entity_list:
            ...
            if len(generated_tagged_index_list) > 0:
                if generated_tagged_index_list[-1] <= 510:
                    """ 
                    exist nested entity, delete shorter one
    ...
    for anno, num in delete_list_dict.items():
        ...
        for anno_item in annotation:
            if anno in anno_item:
                anno_item.remove(anno)
                if len(anno_item)==2:
                    del anno_item[-1]
                if len(anno_item)==0:
                    annotation.remove(anno_item)
        # if len(anno) == 3 and num == 1:
...
def process_annotation_Entity_type(annotation, sent, token_list, entity_type_flag):
    ...
    for anno_item in annotation:
        ...
        for entity in entity_list:
            ...
            if entity_type=="Disease_Symptom":
                entity_type = "Disease"
...
def process_annotation_Relation_type(annotation, sent, token_list, relation, entity_list):
    ...
    for anno_item in annotation:
        if len(anno_item) == 3:
            ...
            if relation_tag =="Outcome-negative" or relation_tag =="Benefit":
                relation_tag = "Drug_Disease_interaction"
...
def process_data(raw_file, tokenizer, res_file, entity_type_list, relation_type_list):
    ...
    print("nested_entity_count", nested_entity_count)
    print("wrong_entity_count", wrong_entity_count)
...
if __name__ == '__main__':
    ...
    # file_list = ["ADE_train.csv"]
    # entity_type_list = ["Drug", "Disease_Symptom"]
    entity_type_list = ["Drug", "Disease"]
    # relation_type_list = ["Benefit", "Outcome-negative"]
    relation_type_list = ["Drug_Disease_interaction"]
