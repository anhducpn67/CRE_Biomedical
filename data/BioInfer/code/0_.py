        ...
        for sentence in document:
            ...
            for entity_or_pair in sentence:
                if entity_or_pair.tag == 'entity':
                    ...
                    entity_id, entity_text, (int(entity_start), int(entity_end)), entity_type)
                    entity_non_relation[entity_id] = [
                        (entity_id, entity_text, (int(entity_start), int(entity_end)), entity_type)]
            pair_list = []
            for entity_or_pair in sentence:
                if entity_or_pair.tag == 'interaction':
                    count_pair += 1
                    pair = entity_or_pair
            ...
            if annotations:
                sentence_count += 1
                f.write(f'66666{sentence_count}||{sentence_text}||{str(annotations)}\n')
            else:
                print("Removed ", sentence_id)
...
def process_1(res_file):
    ...
    file_list = ["BioInfer_train.csv", "BioInfer_valid.csv", "BioInfer_test.csv"]
...
if __name__ == '__main__':
    # Number of pairs: 2534
    # entity_type = ['Individual_protein', 'Protein_family_or_group', 'Gene/protein/RNA', 'Gene', 'DNA_family_or_group',
    #                'Protein_complex']
    # relation_type = ['PPI']
    raw_data = '../raw_data/BioInfer.xml'
    res_file = '../data_csv/BioInfer.csv'
    entity_type = []
    relation_type = []
