...
class My_Ensembled_Classifier(nn.Module):
    ...
    def forward(self, common_embedding):
        ...
        for key in self.TAGS_Types_fileds_dic.keys():
            res = self.get_classifer(key)(common_embedding)
            res_dict[key] = res
            res_list.append(res)
            key_list.append(key)
...
class My_Relation_Classifier(nn.Module):
    ...
    def forward(self, batch_added_marker_entity_span_vec):
        ...
        for sub_task in self.my_relation_sub_task_list:
            res = self.get_classifer(sub_task)(batch_added_marker_entity_span_vec)
            loss_list.append(res)
            res = torch.max(res, 2)
    ...
    def get_entity_pair_span_vec(self, batch_entity, batch_tokens, batch_entity_type, bert_RC):
...
class My_Model(nn.Module):
    ...
    def relation_extraction(self, batch, batch_entity, batch_entity_type):
        ...
        batch_added_marker_entity_vec, batch_entity_pair_span_list, batch_sent_len_list = \
            self.my_relation_classifier.get_entity_pair_span_vec(batch_entity, batch.tokens, batch_entity_type, self.bert_RC)

        batch_pred_raw_res_list, batch_pred_for_loss_sub_task_list = self.my_relation_classifier(batch_added_marker_entity_vec)
