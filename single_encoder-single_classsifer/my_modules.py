...
import copy
from metric import get_triple_O_seg
...
class My_Bert_Encoder(nn.Module):
    ...
    def forward(self, batch_inputs):
        ...
        attention_mask = self.get_attention_mask(tokens_tensor)
        position_ids = self.get_position_ids(tokens_tensor)
        common_embedding = self.bert(tokens_tensor,  attention_mask=attention_mask, position_ids=position_ids)[0]
    ...
    def get_attention_mask(self, tokens_tensor):
        attention_mask = torch.where(tokens_tensor==self.tokenizer.pad_token_id, torch.tensor(0., device=self.device) ,torch.tensor(1., device=self.device))
...
class My_Classifer(nn.Module):
    def __init__(self, output_dim, input_dim, device, ignore_index=None, loss_weight=None):
    ...
    def forward(self, common_embedding, IF_CRF=False):
        # res = self.classfier(common_embedding)
...
class My_Entity_Span_Classifier(nn.Module):
    def __init__(self, args, device):
        super(My_Entity_Span_Classifier, self).__init__()
        self.to(device)
        self.device = device
        self.args = args
    ...
    def forward(self, common_embedding, IF_CRF):
        ...
        for key in self.TAGS_Types_fileds_dic.keys():
            res = self.my_classifer(common_embedding, IF_CRF)
            res_dict[key] = res
            res_list.append(res)
            key_list.append(key)
    ...
    def create_classifers(self, TAGS_Types_fileds_dic):
        ...
        for key, TAGS_fileds in self.TAGS_Types_fileds_dic.items():
            ...
            class_num+=len(TAGS_fileds[1].vocab)

        self.my_classifer = My_Classifer(class_num, self.args.Word_embedding_size, self.device, ignore_index=0)
...
class My_Entity_Span_And_Type_Classifier(nn.Module):
    def __init__(self, args, device):
        super(My_Entity_Span_And_Type_Classifier, self).__init__()
        self.to(device)
        self.device = device
        self.args = args
...
class My_Entity_Type_Classifier(nn.Module):
    def __init__(self, args, device):
        super(My_Entity_Type_Classifier, self).__init__()
        self.to(device)
        self.device = device
        self.args = args
    ...
    def create_classifers(self, TAGS_Types_fileds_dic, TAGS_sep_entity_fileds_dic):
        ...
        self.ignore_index = 0
        self.my_entity_type_sub_task_list.insert(self.ignore_index, "entity_type_PAD")
        self.my_classifer = My_Classifer(len(self.my_entity_type_sub_task_list), self.args.Word_embedding_size, self.device, ignore_index=self.ignore_index)
    ...
    def forward(self, batch_entity_vec, IF_CRF):
        assert IF_CRF==False
        res = self.my_classifer(batch_entity_vec)
        pred_entity_type = torch.argmax(res, dim=2)
        return pred_entity_type, res

    def make_gold_for_loss(self, batch_gold_res_list, batch_entity_token_span_list, my_entity_type_sub_task_list):
        batch_gold_for_loss_sub_task_list = []
        for sent_index, sent_entity_pair in enumerate(batch_entity_token_span_list):
            one_sent_list = []
            for entity in sent_entity_pair:
                find_flag = False
                for sub_task, sub_task_values_list in batch_gold_res_list[sent_index].items():
                    if eval(entity) in sub_task_values_list:
                        find_flag = True
                        aim_class = my_entity_type_sub_task_list.index(sub_task)
                        assert aim_class!=self.ignore_index
                        one_sent_list.append(torch.tensor(aim_class, dtype=torch.long, device=self.device))
                        break

                if not find_flag:
                    one_sent_list.append(torch.tensor(self.ignore_index, dtype=torch.long, device=self.device))
    ...
    def get_ensembled_ce_loss(self, batch_pred_for_loss_sub_task_list, batch_gold_for_loss_sub_task_tensor, ignore_index, weight=None):
        pred_tensor = batch_pred_for_loss_sub_task_list.permute(0,2,1)
        target_tensor = batch_gold_for_loss_sub_task_tensor
...
class My_Relation_Classifier(nn.Module):
    ...
    def create_classifers(self, TAGS_Types_fileds_dic, TAGS_sampled_entity_span_fileds_dic, TAGS_Entity_Type_fileds_dic):
        ...
        self.my_relation_sub_task_list.insert(0, "relation_PAD")
        self.my_relation_sub_task_list.append("relation_None")
        ...
        self.ignore_index = 0
        self.my_classifer = My_Classifer(len(self.my_relation_sub_task_list), self.args.Word_embedding_size*2, self.device, ignore_index=self.ignore_index)

        if self.args.Weight_Loss:
            weight_list = []
            for sub_task in self.my_relation_sub_task_list:
                weight_list.append(self.args.Max_weight)
            weight_list[0] = 0
            weight_list[-1] = self.args.Min_weight

            self.relation_weight = torch.tensor(weight_list, device=self.device)
            print("relation_weight", self.relation_weight)

        else:
            self.relation_weight = None

    def forward(self, batch_added_marker_entity_span_vec, IF_CRF):
        res = self.my_classifer(batch_added_marker_entity_span_vec)
        pred_type_tensor = torch.max(res, 2)[1]
        return pred_type_tensor, res

    def get_entity_pair_rep(self, entity_pair_span, sentence_embedding, Entity_type_TAGS_Types_fileds_dic,
                            dic_map_span_type=None, entity_type_rep_dic=None):
    ...
    def get_entity_pair_span_vec(self, batch_entity, batch_tokens, batch_entity_type, entity_type_rep_dic, bert_encoder,
                                 Entity_type_TAGS_Types_fileds_dic):

        if self.args.Entity_Prep_Way == "entitiy_type_marker":
            raw_batch_entity = copy.deepcopy(batch_entity)
            batch_tokens_marker = []
            for sent_index, (one_sent_tokens, one_sent_span, one_sent_type) in enumerate(
                ...
                for entity_index, (span, type) in enumerate(zip(one_sent_span, one_sent_type)):
                    temp_token_list.insert(span[0], self.tokenizer.convert_tokens_to_ids("[Entity_" + type + "]"))
                    temp_token_list.insert(span[-1] + 2, self.tokenizer.convert_tokens_to_ids("[/Entity_" + type + "]"))
            ...
            common_embedding = bert_encoder(
                pad_sequence(batch_tokens_marker, batch_first=True, padding_value=self.ignore_index))
            ...
            common_embedding = bert_encoder(batch_tokens)
            batch_entity_pair_span_list = []
            batch_entity_pair_vec_list = []
            batch_sent_len_list = []
            ...
            batch_added_marker_entity_span_vec = pad_sequence(batch_entity_pair_vec_list, batch_first=True, padding_value=self.ignore_index)
            ...
            for sent_index, one_sent_entity in enumerate(batch_entity):
                sent_entity_pair_span_list = list(combinations(one_sent_entity, 2))
                batch_sent_len_list.append(len(sent_entity_pair_span_list))
                sent_entity_pair_span_list = [sorted(i) for i in sent_entity_pair_span_list]
    ...
    def make_gold_for_loss(self, gold_all_sub_task_res_list, batch_entity_span_list, my_relation_sub_task_list):
        batch_gold_for_loss_sub_task_list = []
            ...
            for entity_pair in sent_entity_pair:
                ...
                for sub_task, sub_task_values_list in gold_all_sub_task_res_list[sent_index].items():
                    if entity_pair in sub_task_values_list:
                        aim_class = my_relation_sub_task_list.index(sub_task)
                        assert aim_class!=len(my_relation_sub_task_list)-1
                        assert aim_class!=self.ignore_index
                        one_sent_list.append(torch.tensor(aim_class, dtype=torch.long, device=self.device))
                        find_flag = True
                        break
                if not find_flag:
                    one_sent_list.append(torch.tensor(len(my_relation_sub_task_list)-1, dtype=torch.long, device=self.device))
    ...
    def get_ensembled_ce_loss(self, batch_pred_for_loss_sub_task, batch_gold_for_loss_sub_task_tensor):
        pred_tensor = batch_pred_for_loss_sub_task.permute(0,2,1)
        target_tensor = batch_gold_for_loss_sub_task_tensor
        ce_loss = F.cross_entropy(pred_tensor, target_tensor, ignore_index=self.ignore_index, weight=self.relation_weight)
        return ce_loss


    def crossentropy_few_shot(self, batch_pred_for_loss_sub_task, batch_gold_for_loss_sub_task_tensor):
        """
        y_pred don't need softmax
        """
        pred_tensor = batch_pred_for_loss_sub_task
        ...
        fel_ce_loss = F.cross_entropy(y_pred.permute(0,3,2,1), target_tensor, ignore_index=self.ignore_index, weight=self.relation_weight)
        return fel_ce_loss
...
class My_Model(nn.Module):
    def __init__(self, my_Bert_Encoder, args, device):
        super(My_Model, self).__init__()
        self.to(device)
        self.device = device
        self.my_bert_encoder = my_Bert_Encoder
    ...
    def get_entity_type_data(self, batch, common_embedding, valid_test_flag, dic_res_one_batch, epoch):
        if valid_test_flag == 'train' and not self.args.Only_entity_type:
            if epoch<5:
                gold_input_ratio = (50-epoch)/100
            elif epoch<10:
                gold_input_ratio = (30-epoch)/100
            ...
            if gold_input_ratio > random_ratio:
                ...
                for sent_entity_list in get_entity_res_segment(batch_pred_entity_res, TAGS_filed, need_sep_flag=True):
                    ...
                    for entity in sent_entity_list:
                        temp_list.append(str([i[0] for i in entity]))
                    batch_entity.append(temp_list)
        elif valid_test_flag == 'train' and self.args.Only_entity_type:
            TAGS_filed = self.my_entity_type_classifier.TAGS_sep_entity_fileds_dic["sep_entity"][1]
            batch_entity = get_entity_res_segment(batch.sep_entity, TAGS_filed, need_sep_flag=False)
        elif valid_test_flag == 'valid' and  self.args.Only_entity_type:
            TAGS_filed = self.my_entity_type_classifier.TAGS_sep_entity_fileds_dic["sep_entity"][1]
            batch_entity = get_entity_res_segment(batch.sep_entity, TAGS_filed, need_sep_flag=False)
        elif valid_test_flag == 'valid' and  not self.args.Only_entity_type:
            TAGS_filed = self.my_entity_span_classifier.TAGS_Types_fileds_dic["entity_span"][1]
            batch_pred_entity_res = dic_res_one_batch["entity_span"][1]
            batch_entity = []
            for sent_entity_list in get_entity_res_segment(batch_pred_entity_res, TAGS_filed, need_sep_flag=True):
            ...
            raise Exception("entity_type data error !")
    ...
    def get_relation_data(self, batch, valid_test_flag, dic_res_one_batch, epoch):
        ...
        def inner_realtion_data_gold_span():
            ...
            for sent_index, one_sent_entity in enumerate(batch_pred_entity_res):
                ...
                one_sent_temp_list = [eval(TAGS_filed.vocab.itos[int(i)]) for i in one_sent_temp_list.tolist()]
                batch_entity.append(sorted(one_sent_temp_list, key=lambda s: s[0]))
                if 'entity_type' in self.args.Task_list:
        ...
        def inner_realtion_data_gold_span_and_type():
            TAGS_filed = self.my_entity_span_and_type_classifier.TAGS_Types_fileds_dic['entity_span_and_type'][1]
            batch_pred_entity_res = batch.entity_span_and_type

            for sent_index, one_sent_entity in enumerate(batch_pred_entity_res):
                one_sent_temp_list = one_sent_entity[:get_sent_len(one_sent_entity, TAGS_filed)]  # deal with PAD
                one_sent_temp_list = [TAGS_filed.vocab.itos[int(i)] for i in one_sent_temp_list.tolist()]

                temp_span = []
                temp_type = []
                for entity in  get_triple_O_seg(one_sent_temp_list):
                    temp_span.append([i[0] for i in entity])
                    temp_type.append(entity[0][1].split("_")[0])

                batch_entity.append(temp_span)
                batch_entity_type.append(temp_type)
            ...
            for sent_index in range(len(batch)):
                ...
                for index, (key, sent_entity_list) in enumerate(batch_pred_entity_res_dic.items()):
                    for entity_pair in sent_entity_list[sent_index]:
                        ...
                        one_sent_type_list.append(entity_pair[0][1].split("_")[0])

                one_sent_temp_list = sorted(one_sent_all_type_span_list, key=lambda s: s[0])
                batch_entity.append(one_sent_temp_list)
                batch_entity_type.append(one_sent_type_list)
            ...
            if self.args.Only_relation:
                ...
                if gold_input_ratio > random_ratio:
                    if self.args.Relation_input == "entity_span":
                        ...
                        inner_realtion_data_gold_span_and_type()
                    else:
        ...
        if not self.args.Only_relation:
            assert len(batch_entity)==len(batch_entity_type)

        return batch_entity, batch_entity_type

    def forward(self, batch, epoch, entity_type_rep_dic, valid_test_flag='train'):
        dic_res_one_batch ={}
        dic_loss_one_batch ={}

        common_embedding = self.my_bert_encoder(batch.tokens)

        if  "entity_span" in self.task_list:
            batch_gold_and_pred_entity_res, entity_span_batch_loss = self.entity_span_extraction(batch, common_embedding)
        ...
        if "entity_type" in self.task_list:
            batch_entity_vec_list, batch_entity_token_span_list = self.get_entity_type_data(batch, common_embedding, valid_test_flag, dic_res_one_batch, epoch)
            batch_gold_and_pred_entity_type_res, one_batch_entity_type_loss = self.entity_type_extraction(batch, batch_entity_vec_list, batch_entity_token_span_list, valid_test_flag)
        ...
        if "entity_span_and_type" in self.task_list:
            sub_task_batch_gold_and_pred_entity_span_and_type_res, batch_entity_span_and_type_loss = self.entity_span_and_type_extraction(batch, common_embedding)
        ...
        if "relation" in self.task_list:
            batch_entity, batch_entity_type = self.get_relation_data(batch, valid_test_flag, dic_res_one_batch, epoch)
            one_batch_relation_res, one_batch_relation_loss = self.relation_extraction(batch, batch_entity, batch_entity_type, entity_type_rep_dic)
    ...
    def entity_type_extraction(self, batch, batch_entity_vec_list, batch_entity_token_span_list, valid_test_flag):
        ...
        for sent_index in range(len(batch)):
            ...
            for sub_task_index, sub_task in enumerate(self.my_entity_type_classifier.my_entity_type_sub_task_list):
                ...
                for entity_span, pred_type in zip(batch_entity_token_span_list[sent_index], batch_pred_raw_res_list[sent_index]):
                ...
                if sub_task != 'entity_type_PAD':
                    for entity_pair in getattr(batch, sub_task)[sent_index]:
                        ...
                        if str(entity_pair_span) != '[PAD]':
                            gold_one_sent_all_sub_task_res_dic[sub_task].append(eval(entity_pair_span))
            batch_gold_res_list.append(gold_one_sent_all_sub_task_res_dic)

        batch_gold_for_loss_sub_task_tensor = self.my_entity_type_classifier.make_gold_for_loss(
            batch_gold_res_list, batch_entity_token_span_list, self.my_entity_type_classifier.my_entity_type_sub_task_list)
    ...
    def relation_extraction(self, batch, batch_entity, batch_entity_type, entity_type_rep_dic):
        """ Relation extraction """
        if "entity_type" in self.args.Task_list:
            total_entity_type_to_index_dic = self.my_entity_type_classifier.total_entity_type_to_index_dic
        else:
            total_entity_type_to_index_dic = None
        ...
        batch_added_marker_entity_vec, batch_entity_pair_span_list, batch_sent_len_list = \
            self.my_relation_classifier.get_entity_pair_span_vec(batch_entity, batch.tokens, batch_entity_type,
                                                                 entity_type_rep_dic, self.my_bert_encoder,
                                                                 total_entity_type_to_index_dic)
        batch_pred_raw_res_list, batch_pred_for_loss_sub_task = self.my_relation_classifier(batch_added_marker_entity_vec, self.IF_CRF)
        ...
        for sent_index in range(len(batch)):
            ...
            for sub_task in self.my_relation_classifier.my_relation_sub_task_list:
                gold_one_sent_all_sub_task_res_dic.setdefault(sub_task, [])
                if sub_task != "relation_None" and sub_task != "relation_PAD":
                    for entity_pair in getattr(batch, sub_task)[sent_index]:
                        entity_pair_span = self.my_relation_classifier.TAGS_Types_fileds_dic[sub_task][1].vocab.itos[entity_pair]
                        if str(entity_pair_span) != '[PAD]':
                            gold_one_sent_all_sub_task_res_dic[sub_task].append(sorted(eval(entity_pair_span)))
            batch_gold_res_list.append(gold_one_sent_all_sub_task_res_dic)
        ...
        for sent_index in range(len(batch)):
            ...
            for sub_task_index, sub_task in enumerate(self.my_relation_classifier.my_relation_sub_task_list):
                ...
                for entity_pair_span, pred_type in zip(batch_entity_pair_span_list[sent_index][:sent_len], batch_pred_raw_res_list[sent_index][:sent_len]):
                    if pred_type == sub_task_index:
                        # pred_one_sent_all_sub_task_res_dic[sub_task].append(sorted(eval(entity_pair_span)))
        ...
        batch_gold_for_loss_sub_task_tensor = self.my_relation_classifier.make_gold_for_loss(
            batch_gold_res_list, batch_entity_pair_span_list, self.my_relation_classifier.my_relation_sub_task_list)

        if self.my_relation_classifier.args.Loss == "FSL":
            one_batch_relation_loss = self.my_relation_classifier.crossentropy_few_shot(batch_pred_for_loss_sub_task, batch_gold_for_loss_sub_task_tensor)
        elif self.my_relation_classifier.args.Loss == "CE":
            one_batch_relation_loss = self.my_relation_classifier.get_ensembled_ce_loss(batch_pred_for_loss_sub_task, batch_gold_for_loss_sub_task_tensor)
        else:
