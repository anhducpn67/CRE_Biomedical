...
def get_corpus_file_dic(all_data_flag, corpus_list, Task_list):
    ...
    # new file address
    if all_data_flag:
        combining_data_files_list = [os.path.join('../data', 'Multi_Task_Training', 'base', str(corpus_list)+'_train_base_model_data.json'),
                                     os.path.join('../data', 'Multi_Task_Training', 'base', str(corpus_list)+'_valid_base_model_data.json'),
                                     os.path.join('../data', 'Multi_Task_Training', 'base', str(corpus_list)+'_test_base_model_data.json')]
    else:
        combining_data_files_list = [os.path.join('../data', 'Multi_Task_Training', 'base', 'test', str(corpus_list)+'_train_base_model_data.json'),
                                     os.path.join('../data', 'Multi_Task_Training', 'base', 'test', str(corpus_list)+'_valid_base_model_data.json'),
                                     os.path.join('../data', 'Multi_Task_Training', 'base', 'test', str(corpus_list)+'_test_base_model_data.json')]
    ...
    if temp_flag==0:
        ...
        for corpus_name, corpus_inform in pick_corpus_file_dic.items():
            ...
            for index, raw_train_valid_test_file in enumerate(corpus_train_valid_test_file):
                if all_data_flag:
                    raw_train_valid_test_file = os.path.join('../data', corpus_name, 'BIOES', 'base', raw_train_valid_test_file)
                else:
                    raw_train_valid_test_file = os.path.join('../data', corpus_name, 'BIOES', 'base', 'test', raw_train_valid_test_file)
    ...
    return return_corpus_file_dic, sep_corpus_file_dic


# def prepared_data(BATCH_SIZE, device, tokenizer, file_train_valid_test_list, entity_type_num_list, relation_list):
#
#     ID_fileds = torchtext.data.Field(dtype=torch.long, batch_first=True, use_vocab=False, sequential=False)
...
#
#     # ratio_relation_dic, yes_no_relation_dic = statistics_corpus(train_file, [i[0] for i in TAGS_Relation_pair_fileds_dic.values()])
#
...
#         [train_set, valid_set, test_set], batch_size=BATCH_SIZE, sort=True, shuffle=True,
...
#            TAGS_entity_span_fileds_dic, TAGS_only_Entity_Type_fileds_dic, TAGS_joint_Entity_Type_fileds_dic, TAGS_Relation_pair_fileds_dic, \
#            TAGS_sampled_entity_span_fileds_dic, TAGS_sep_entity_fileds_dic

def prepared_data_1(BATCH_SIZE, device, tokenizer, file_train_valid_test_list, entity_type_num_list, relation_list):
    ...
    TAGS_joint_Entity_Type_fileds = torchtext.data.Field(dtype=torch.long, batch_first=True, pad_token= tokenizer.pad_token, unk_token=None)
    TAGS_joint_Entity_Type_fileds_dic = {"entity_span_and_type":("entity_span_and_type",TAGS_joint_Entity_Type_fileds)}

    TAGS_only_Entity_Type_fileds_dic = {}
    TAGS_only_Entity_Type_fileds = torchtext.data.Field(dtype=torch.long, batch_first=True, pad_token= tokenizer.pad_token, unk_token=None)
    for entity in entity_type_num_list:
        TAGS_only_Entity_Type_fileds_dic["only_entity_type_"+entity] = ("only_entity_type_"+entity,TAGS_only_Entity_Type_fileds )

    TAGS_Relation_pair_fileds_dic = {}
    TAGS_Relation_pair_fileds = torchtext.data.Field(dtype=torch.long, batch_first=True, pad_token= tokenizer.pad_token, unk_token=None)
    for relation in relation_list:
        TAGS_Relation_pair_fileds_dic["relation_"+relation] = ("relation_"+relation, TAGS_Relation_pair_fileds )
    ...
    TAGS_sep_entity_fileds.build_vocab(train_set, valid_set, test_set)
    TAGS_sampled_entity_span_fileds.build_vocab(train_set, valid_set, test_set)
    # TAGS_joint_Entity_Type_fileds.build_vocab(train_set, valid_set, test_set, specials=["S", "B", "I", "E"])
    TAGS_joint_Entity_Type_fileds.build_vocab(train_set, valid_set, test_set)
    TAGS_only_Entity_Type_fileds.build_vocab(train_set, valid_set, test_set)
    TAGS_Relation_pair_fileds.build_vocab(train_set, valid_set, test_set)
