...
def report_entity_span_PRF(list_batches_res, TAGS_fileds_list, dic_sub_task_corpus, improve_flag, corpus_list):
    ...
    for one_batch_gold_list, one_batch_res_list in list_batches_res:
        for one_sent_pred, one_sent_gold in zip(one_batch_res_list, one_batch_gold_list):
            ...
            pred_list = improved_result(get_triple_O_seg(pred_sent)) if improve_flag else get_triple_O_seg(pred_sent)
    ...
    for corpus, task_dic in sep_corpus_file_dic.items():
        ...
        for task, sub_task_list in task_dic.items():
            ...
            for sub_task in sub_task_list:
                ...
                dic_sub_task_corpus.setdefault("entity_span_and_type", [])
                dic_sub_task_corpus[sub_task].append(corpus)
                dic_sub_task_corpus["entity_span_and_type"].append(corpus)
...
def make_relation_test_data(TAGS_Relation_fileds_dic):
    ...
    gold_pair_list = [
        ...
        ['([24, 25, 26, 27], [80, 81])'],
        ['([102, 103, 104], [5, 6, 7])', '([5, 6, 7], [45, 46, 47, 48])'],
    ]
    for gold_index, gold_pair in zip(gold_index_list, gold_pair_list):
        gold_res[key_list[gold_index]].extend(gold_pair)
...
if __name__ =="__main__":
    ...
    from data_loader import prepared_data, get_corpus_file_dic
    import argparse
    ...
    parser.add_argument('--Task_list', default=["entity_span", "entity_type"],
                        nargs='+', help=["entity_span", "entity_type", "entity_span_and_type", "relation"])

    parser.add_argument('--Corpus_list', default=["DDI"], nargs='+', help=["CPR", "DDI", "CDR"])
    parser.add_argument('--Train_way', default="Multi_Task_Training", type=str,
                        help=["Multi_Task_Training",
                              "Incremental_Training_freeze", "Incremental_Training_unfreeze",
                              "Multi_Task_Incremental_Training_freeze", "Multi_Task_Incremental_Training_freeze"
                              ])

    parser.add_argument('--Entity_Prep_Way', default="standard_entity", type=str,
                        help=["standard_entity", "entitiy_marker", "entitiy_type_marker", "trained_entitiy_type_marker"])
    parser.add_argument('--Pair_Combine_Way', default="mentions_pooling", type=str,
                        help=["entitiy_pair", "mentions_pooling", "marker_start", "mention_pooling_marker_start"])

    parser.add_argument('--LR_bert', default=1e-5, type=float)
    parser.add_argument('--LR_classfier_entity_span', default=2e-4, type=float)
    parser.add_argument('--LR_classfier_entity_type', default=2e-3, type=float)
    parser.add_argument('--LR_classfier_entity_span_and_type', default=1e-3, type=float)
    parser.add_argument('--LR_classfier_relation', default=1e-3, type=float)
    ...
    parser.add_argument('--Hard_Restarts_num', default=2, type=float, help="1 means no restart")
    parser.add_argument('--Min_train_performance_Report', default=0, type=int)
    parser.add_argument('--Min_valid_num', default=9990, type=int)
    ...
    parser.add_argument('--Loss', type=str, default="BCE", help=["BEC", "CE", "FSL"])
    parser.add_argument('--Tau', default=1.0, type=float)
    parser.add_argument('--Relation_tag_weight_lambda', default=5, type=float)
    ...
    for corpus_name, (entity_type_num_list, relation_num_list, file_train_valid_test_list) in corpus_file_dic.items():
        train_iterator, valid_iterator, test_iterator, TOEKNS_fileds, \
        ...
        ratio_relation_dic, yes_no_relation_dic, TAGS_sampled_entity_span_fileds_dic, TAGS_sep_entity_fileds_dic= \
            prepared_data(4, device, tokenizer, file_train_valid_test_list, entity_type_num_list, relation_num_list)

        dic_batches_res["entity_span"] = make_entity_span_test_data(TAGS_entity_span_fileds_dic)
        dic_batches_res["entity_type"] = make_entity_type_test_data(TAGS_only_Entity_Type_fileds_dic)
        dic_batches_res["entity_span_and_type"] = make_entity_span_and_type_test_data(TAGS_joint_Entity_Type_fileds_dic)
        ...
        my_entity_type_classifier.create_classifers(TAGS_only_Entity_Type_fileds_dic, TAGS_sep_entity_fileds_dic)
        my_entity_span_and_type_classifier.create_classifers(TAGS_joint_Entity_Type_fileds_dic)
        my_relation_classifier.create_classifers(TAGS_Relation_pair_fileds_dic, ratio_relation_dic, yes_no_relation_dic, TAGS_sampled_entity_span_fileds_dic)
