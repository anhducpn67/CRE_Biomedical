...
def get_entity_type_rep_dic(train_iterator, valid_iterator, my_model, device):
    my_entity_type_sub_task_list = my_model.my_entity_type_classifier.my_entity_type_sub_task_list
    word_embeddings = my_model.my_bert_encoder.bert.embeddings.word_embeddings.weight
    ...
    for k,v in entity_type_rep_dic.items():
        entity_type_rep_dic[k] = torch.mean(torch.stack(v), dim=0).data

    entity_type_rep_dic["None"] = torch.FloatTensor([0] * word_embeddings.shape[1]).to(device)
        ...
        for batch_index in range(batch_num):
            for sent_index, sent_ID in enumerate(dic_batches_res["ID_list"][batch_index]):
                ...
                if "entity_type" in task_list:
                    ...
                    res_dic["entity_type"] = {"gold_all_sub": gold_all_sub_task_dic, "pred_all_sub":pred_all_sub_task_dic}
                ...
                if "entity_span_and_type" in task_list:
                    ...
                    res_dic["entity_span_and_type"] = {"gold_all_sub": dic_gold_entity_span_and_type_res, "pred_all_sub":dic_pred_entity_span_and_type_res}
                ...
                if "relation" in task_list:
                    ...
                    res_dic["relation"] = {"gold_all_sub": gold_all_sub_task_dic, "pred_all_sub":pred_all_sub_task_dic}
...
def get_entity_res_segment(one_batch_res, TAGS_filed, need_sep_flag):
    ...
    for sent_index, one_sent_tags in enumerate(one_batch_res):
        ...
        sentence = [TAGS_filed.vocab.itos[int(i)] for i in sentence.tolist()]
        if  need_sep_flag:
