...
parser.add_argument('--Corpus_list', default=["DDI", "CPR", "Twi_ADE", "ADE"], nargs='+', help=["CPR", "DDI", "Twi_ADE", "ADE"])
parser.add_argument('--Random_ratio', default=1, type=float, help=">1 means mask all data from other corpus")
parser.add_argument('--ratio_CPR', default=1, type=float)
parser.add_argument('--Group_num', default=40, type=int)
parser.add_argument('--Test_flag', action='store_true', default=True, help=[False, True])
parser.add_argument('--Test_TAC_flag', action='store_true', default=True, help=[False, True])  #"TAC2019"
parser.add_argument('--Inner_test_TAC_flag', action='store_true', default=True, help=[False, True])
parser.add_argument('--Test_Corpus', default=["TAC2019"], nargs='+', help=["ADE", "Twi_ADE", "DDI", "CPR", "TAC2019"])  #TAC
parser.add_argument('--Test_model_file', type=str, default="../result/save_model/Model_['--ID', '77774', '--GPU', '1', '--All_data', '--Corpus_list', 'CPR', 'DDI', 'Twi_ADE', 'ADE', '--Entity_Prep_Way', 'entitiy_type_marker', '--ratio_CPR', '1']")

parser.add_argument('--Entity_Prep_Way', default="entitiy_type_marker", type=str, help=["standard", "entitiy_type_marker", "entity_type_embedding", "soft_sharing"])
...
class Train_valid_test():
    def __init__(self, data_ID_2_corpus_dic, my_model, tokenizer_list,
        ...
        self.valid_iterator_list = valid_iterator_list
        self.test_iterator_list = test_iterator_list


        self.train_iterator_dic = self.get_dic_data(train_iterator_list, args.Group_num, "train")
        self.valid_iterator_dic = self.get_dic_data(valid_iterator_list, 1, "valid")
        ...
        if args.If_add_prototype:
            self.entity_type_rep_dic = get_entity_type_rep_dic(self.train_iterator_list, self.valid_iterator_list, self.my_model, args.Type_emb_num, device)
        else:
    ...
    def get_dic_data(self, iterator_list, group_num, data_flag):
        ...
        if args.Test_TAC_flag and (not args.Inner_test_TAC_flag) and (data_flag=="train" or data_flag=="valid"):
            ...
            for corpus_name in corpus_list:
                len_raw_list = len(NER_data_dic[corpus_name])
                if int(len_raw_list / group_num) <= 0 :
                    step_len = 1
                else:
                    step_len = int(len_raw_list / group_num)

                NER_group = [NER_data_dic[corpus_name][i:i + step_len] for i in range(0, len_raw_list, step_len)]
                RC_group = [RC_data_dic[corpus_name][i:i + step_len] for i in range(0, len_raw_list, step_len)]
                return_data_dic[corpus_name] = [[ner_data, rc_data] for ner_data, rc_data in zip(NER_group, RC_group)]

            if data_flag=="train" and "CPR" in corpus_list:
                for stage_data in return_data_dic["CPR"]:
                    stage_data[0] = stage_data[0][0:int(len(stage_data[0])/args.ratio_CPR)]
                    stage_data[1] = stage_data[1][0:int(len(stage_data[1])/args.ratio_CPR)]
    ...
    def one_epoch(self, iterator_dic, valid_test_flag, epoch):
        ...
        if args.All_data:
            group_num = args.Group_num+1
        else:
            group_num = args.Group_num
        if valid_test_flag=="valid":
            group_num=1
        ...
        for group_number in range(group_num):
            for corpus_name in corpus_list:
                ...
                if corpus_name in iterator_dic.keys() and group_number<len(iterator_dic[corpus_name]):
                    ...
                    group_list_NER_RC = iterator_dic[corpus_name][group_number]
                    if "relation" in args.Task_list:
        ...
        for epoch in range(self.current_epoch, args.EPOCH+1):
            ...
            if epoch >= args.Min_train_performance_Report :
                ...
                if args.Inner_test_TAC_flag:
                    ...
                    corpus_list = ["TAC2019"]
                    sep_corpus_file_dic = {"TAC2019":self.sep_corpus_file_dic["TAC2019"]}
                else:
                    sep_corpus_file_dic = self.sep_corpus_file_dic
                ...
                if dic_valid_PRF[args.Task_list[-1]][2] >= maxF:
                    ...
                    recored_detail_performance(epoch, dic_valid_total_sub_task_P_R_F, dic_valid_PRF, file_detail_performance,
                                               ...
                                               sep_corpus_file_dic, args.Task_list, corpus_list, args.Average_Time)
                else :
                    early_stop_num -= 1
    ...
    def test_fn(self, file_model_save):
        ...
        with torch.no_grad():
            ...
            for group_number in range(group_num):
                for corpus_name in corpus_list:
                    group_list_NER_RC = self.test_iterator_dic[corpus_name][group_number]
                    if "relation" in args.Task_list:
                        temp_my_iterator_list = [ [ner, rc] for ner, rc in zip(group_list_NER_RC[0], group_list_NER_RC[1]) ]
                    else:
                        # only relation have a bug
            ...
            dic_loss["average"] = epoch_loss / len(self.my_model.task_list)
            classifiers_dic = self.my_model.classifiers_dic
            sep_corpus_file_dic = {"TAC2019":self.sep_corpus_file_dic["TAC2019"]}
            ...
            # record_pred_str_res(self.my_model.task_list, file_result, dic_batches_res, 0,
            #                     self.tokenizer_list[0].convert_ids_to_tokens, self.my_model.classifiers_dic,
            #                     args.IF_CRF)
